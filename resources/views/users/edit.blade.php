                ...
                <form action="{{ route('users.store') }}" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" name="password_confirmation" class="form-control"
                            id="password_confirmation" value="{{old('password')}}" placeholder="Enter Password">
                    </div>
                    ...
                    {{-- {{dd($user->password)}} --}}
                    @if(!empty( $user->id ))
                    @if((!empty ( $user->password ) && Auth::user()->hasRole('user')) || (
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" id="avatar" class=" form-control @error('avatar') is-invalid @enderror"
                            name="avatar">
                        @if(!empty($user->id))
                    ...
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    @if(empty($user->id) || empty($userRole) )
                    <div class="form-group">
                        <a class="btn btn-primary  mb-3 mt-3" data-toggle="collapse" href="#collapseExample"
                        ...
                        <div class="collapse" id="collapseExample">
                            <div class="card card-body">
                                @foreach($roles as $role)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" name="roles[]" id="inlineRadio"
                                                value="{{ $role }}" {{ in_array( $role, $userRole ) ? 'checked' : '' }}>
                                            <label class="ford-check-label" for="inlineRadio"> {{ $role}} </label>
                                        </div>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                <ul class="list-group w-45">
                                    ...
                                    <li
                                        ...
                                        <input type="hidden" value="{{$user_subject->name}}"
                                            name="subject_name_hidden[]">
                                        <input type="hidden" value="{{$user_subject->pivot->subject_id}}"
                                            name="subject_id[]">
                                        <input type="hidden" value="{{$user_subject->pivot->point}}" name="point[]">
                                        <button class="remove btn btn-link "><span
                                                class="badge badge-primary badge-pill"><i class="fa fa-times"
            ...
            <div class="w-50 ml-5">
                <p>
                    ...
                    <button class="btn btn-primary" type="button" data-toggle="collapse"
                        ...
                        aria-controls="multiCollapseExample2">Các
                        môn chưa học</button>
